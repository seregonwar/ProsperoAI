...
- **Gateway OpenAI-compatible (§26)**: `/healthz`, `/v1/models` (con metadati reali `kind`/`size_bytes`/`endpoint`/`broken`), `/v1/chat/completions` in **streaming SSE**. Avvio: `pai serve <model.pai> [--remote name@host:port]... --host 127.0.0.1 --port 8080`.
